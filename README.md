...
2. Engineering Problem Statement  
   - Deploying AI models is not only about achieving accurate predictions, but about ensuring that inference services are reliable, observable, and recoverable under failure conditions.  A
...
7. Troubleshooting (Real Incidents)  
   - Encountered a corrupted Kubernetes binary caused by downloading an HTML file instead of the executable. Identified the issue by inspecting the file header and resolved it by reinstalling from the official distribution source.  Kubernetes 
...
8. Security & Production Readiness  
   - Designed the system with non-root container execution in mind  비루트(non-root) 
